...
public class ConnectedPlayer implements MinecraftConnectionAssociation, Player {
    ...
    private Optional<RegisteredServer> getNextServerToTry(@Nullable RegisteredServer current) {
        return server.getAllServers().stream().filter(registeredServer -> !((connectedServer != null && hasSameName(connectedServer.getServer(), registeredServer.getServerInfo().getName()))
                || (connectionInFlight != null && hasSameName(connectionInFlight.getServer(), registeredServer.getServerInfo().getName()))
                || (current != null && hasSameName(current, registeredServer.getServerInfo().getName())))).findAny();
    }
